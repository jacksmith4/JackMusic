...
async def getSettings(bot: commands.Bot, data: Dict) -> Dict:
    ...
    guild = bot.get_guild(guild_id)
    if not guild:
        return error_msg("RhythmoSync don't have access to requested guild.", user_id=user_id, level="error")
...
async def updateSettings(bot: commands.Bot, data: Dict) -> None:
    ...
    guild = bot.get_guild(guild_id)
    if not guild:
        return error_msg("RhythmoSync don't have access to required guild.", user_id=user_id, level="error")
